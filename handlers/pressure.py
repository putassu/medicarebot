...
class Pressure(StatesGroup):
    ...
    comment = State()
    check_state = State()
    import_pressure = State()
#     items = State()
#     add_more = State()
#     photo_id = State()
#     check_state = State()

pressure_router = Router()
user_messages = {}
pressure_router.message.filter(lambda message: message.photo or (message.text and not message.text.startswith('/')))

@pressure_router.callback_query(F.data == 'new_pressure', Pressure.manual)
async def cmd_new_pressure(call: CallbackQuery, state: FSMContext):
    await state.clear()
    await call.answer('Внесите новое значение давления')
    sent_message = await call.message.answer('Введите систолическое давление', reply_markup=None)
...
@pressure_router.message(F.text, Pressure.systolic)
async def cmd_new_pressure(message: Message, state: FSMContext):
    await state.update_data(user_id=message.from_user.id)
    systolic = extract_number(message.text)
    if not systolic:
        sent_message = await message.answer('Введите систолическое давление корректно!', reply_markup=None)
        user_messages[message.from_user.id] = [sent_message.message_id]
        return
    await state.update_data(systolic=systolic)
    sent_message = await message.answer('Введите диастолическое давление', reply_markup=None)
    user_messages[message.from_user.id] = [sent_message.message_id]
    await state.set_state(Pressure.diastolic)
